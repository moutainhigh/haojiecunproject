...
public class InvestAccount extends BaseController {
	...
	public static void debtDetailsFailure(String sign){
		...
		Debt debtBussiness = new Debt();
		debtBussiness.id = viewDebtId;
		String sql = "SELECT SUM(real_receive_corpus+real_receive_interest) FROM t_bill_invests where bid_id = ?";
		Double monery = t_bill_invests.find(sql,viewDebtId).first();
		
		render(debt,debtBussiness,monery);
	}
	 ...
	 * @param pageNum
	 */
	public static void redeem(String investSign){
	    ErrorInfo error = new ErrorInfo();
	   
	    long investId = Security.checkSign(investSign, Constants.BID_ID_SIGN, Constants.VALID_TIME, error);
        ...
        invest.id=investId;
        if(new Date().compareTo(invest.bid.investExpireTime)<0){
        	error.msg="定期标未过锁定期，不能赎回";
 	        flash.error(error.msg);
 	        repayingInvestBid(0,null);
        }
	    int row=0;
		String hql = "update t_invests set redeem_status=?, redeem_time=? where id=? ";

		Query query = JPA.em().createQuery(hql);
		query.setParameter(1, Constants.ONE);// 赎回状态  0 赎回 1赎回申请中 2赎回成功
		query.setParameter(2, new Date());// 赎回申请时间
		query.setParameter(3, investId);// 条件
		
		try {
			row = query.executeUpdate();
		} catch (Exception e) {
			e.printStackTrace();
			Logger.error("定期标->赎回申请:" + e.getMessage());
			error.msg = "修改赎回申请状态失败!";
			JPA.setRollbackOnly();
	...
	public static void debtTransfer(String sign){
		...
		t_bill_invests bill = t_bill_invests.find(" invest_id = ?", investId).first();
		render(invest,map, bill);
	}
	/**
	 * 取消债权转让
	 * @param investId
	 */
	@DebtCheck(1)
	public static void cancelDebtTransfer(String sign){
		...
		int resulta =Debt.cancelHasBidTransfer(investId);
		if(resulta < 0){
			error.code = -7;
			error.msg = "取消债权转让异常";
...
//			Cache.add(pMerBillNo, debtId, IPSConstants.CACHE_TIME);
	...
	public static void saveOrUpdateRobot(int validType, int validDate, double minAmount, double maxAmount){
		...
		if( StringUtils.isBlank(rateStart) || StringUtils.isBlank(rateEnd)){
			msg = "请正确设置各种参数！";
