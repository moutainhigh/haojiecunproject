...
public class AccountHome extends BaseController {
	...
	public static void home() {
        ...
        List<v_receiving_invest_bids> vbid = Invest.queryUserReceivingInvestBids(user.id,error); 
        //根据用户id查询用户的体验金余额
        Bbin bb = new Bbin();
        t_bbin bb2 = bb.queryBbinforInvest(user.id, error);
		render(bb2,user, warmPrompt,unReadMsgCount,news,backstageSet, content,sumInterest,total,accountInfo,vbid);
	}
	...
	public static void loanAccount(){
		...
		render(user, accountStatistics, news, fundraiseingBid, toSubmitItemBid, recentlyRepayBills,backstageSet, content,sumInterest,total,accountInfo);
	}
	 ...
	 */
	@SubmitOnly
	public static void submitRepayment(String payPassword, double amount, String billId,String bidTitle){
		if(bidTitle.contains("体验")){
