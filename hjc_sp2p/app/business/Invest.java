...
public class Invest implements Serializable{
	...
	public int loanBidStatus;
	public int redeemStatus;
	public long redeemAuditId;
	...
	public void setId(long id) {
		...
		this.loanBidStatus = invests.loan_bid_status;
		this.redeemAuditId = invests.redeem_audit_id;
		this.redeemStatus = invests.redeem_status;
		this.redeemTime = invests.redeem_time;
	}
	 ...
	 * @return
	 */
	public static PageBean<v_return_rate_records> queryBidRateRecords(int currPage, int pageSize,long bidId,ErrorInfo error){
		
		PageBean<v_return_rate_records> pageBean = new PageBean<v_return_rate_records>();
		List<v_return_rate_records> list = new ArrayList<v_return_rate_records>();
		pageBean.currPage = currPage;
		pageBean.pageSize = pageSize;
		StringBuffer sql = new StringBuffer("");
		sql.append(SQLTempletes.PAGE_SELECT);
		sql.append(SQLTempletes.V_RATE_RECORDS);   
		sql.append(" where r.bid_id = ? ");
		sql.append("");
		List<Object> params = new ArrayList<Object>();
		params.add(bidId);
		...
		try {
			EntityManager em = JPA.em();
            Query query = em.createNativeQuery(sql.toString(),v_return_rate_records.class);
            for(int n = 1; n <= params.size(); n++){
                query.setParameter(n, params.get(n-1));
            }
            query.setFirstResult((currPage - 1) * pageSize);
	 ...
	 * @return
	 */
	public static PageBean<v_front_all_bids> queryAllRegularBids(int showType, int currPage,int pageSize,String _apr,String _amount,String _loanSchedule,String _startDate,String _endDate,String _loanType,String minLevelStr,String maxLevelStr,String _orderType,String _keywords,ErrorInfo error){
		...
		sql.append(" and t_bids.is_regular = true ");
		List<Object> params = new ArrayList<Object>();
		...
		if(StringUtils.isBlank(_apr) && StringUtils.isBlank(_amount) && StringUtils.isBlank(_loanSchedule) && StringUtils.isBlank(_startDate) && StringUtils.isBlank(_endDate) && StringUtils.isBlank(_loanType) && StringUtils.isBlank(minLevelStr) && StringUtils.isBlank(maxLevelStr) && StringUtils.isBlank(_orderType) &&  StringUtils.isBlank(_keywords)){
			...
			try {
				...
				sql.append(" order by loan_schedule asc,is_hot desc,id desc");
	...
	public static PageBean<v_front_all_bids> queryAllBids(int showType, int currPage,int pageSize,String _apr,String _amount,String _loanSchedule,String _startDate,String _endDate,String _loanType,String minLevelStr,String maxLevelStr,String _orderType,String _keywords,ErrorInfo error){
		...
		sql.append(SQLTempletes.SELECT);
		sql.append(SQLTempletes.V_FRONT_ALL_BIDS);
		sql.append(" and t_bids.is_regular = false ");
	 ...
	 * @return
	 */
public static PageBean<v_front_all_bids> queryAllBbids(int showType, int currPage,int pageSize,String _apr,String _amount,String _loanSchedule,String _startDate,String _endDate,String _loanType,String minLevelStr,String maxLevelStr,String _orderType,String _keywords,ErrorInfo error){
		...
		sql.append(SQLTempletes.V_FRONT_ALL_BIDS);
		sql.append(" and t_bids.is_regular = false ");
		sql.append(" and t_bids.title like '%体验%'");
	 ...
	 * @return
	 */
public static PageBean<v_front_all_bids> queryAllBidss(int showType, int currPage,int pageSize,String _apr,String _amount,String _loanSchedule,String _startDate,String _endDate,String _loanType,String minLevelStr,String maxLevelStr,String _orderType,String _keywords,ErrorInfo error){
	...
	sql.append(SQLTempletes.V_FRONT_ALL_BIDS);
	sql.append(" and t_bids.is_regular = false ");
	sql.append(" and t_bids.title not like '%体验%'");
	List<Object> params = new ArrayList<Object>();
	...
	public static PageBean<v_invest_records> queryUserInvestRecords(long userId, String currPageStr,String pageSizeStr,String typeStr,String paramter,ErrorInfo error){
		...
		sql.append(SQLTempletes.V_INVEST_RECORDS);
		sql.append(" and t_invests.user_id=? and t_bids.status != -4");
		List<Object> params = new ArrayList<Object>();
		params.add(userId);
	...
	public static PageBean<v_invest_records> queryInvestRecords(long userId, String currPageStr, String pageSizeStr, ErrorInfo error){
		...
		sql.append(SQLTempletes.PAGE_SELECT);
		sql.append(SQLTempletes.V_INVEST_RECORDS);
		sql.append(" and user_id = ? and t_bids.status != -4 ");
		sql.append(" order by time desc");
		List<Object> params = new ArrayList<Object>();
		params.add(userId);
		try {
	...
	public static int saveOrUpdateRobot(long userId, int validType, int validDate, double minAmount, double maxAmount, String bidAmount,String rateStart,String rateEnd,String deadlineStart,String deadlineEnd,String creditStart,String creditEnd,
		...
		if(robot == null){
			...
			if(null == remandAmount){
				error.msg = "对不起！您可投资金额不能为空！";
				error.code = -6;
				return error.code;
			}
			if(0 > Double.parseDouble(remandAmount)){
				error.msg = "对不起！您可投资金额不能小于0！";
				error.code = -10;
				return error.code;
			}
			//算出账户保留金额
			double reminMoney = balance -Double.parseDouble(remandAmount);
			if(reminMoney <= 0){
				error.msg = "对不起！您的可投资金额不能大于您的可用余额！";
				error.code = -4;
				return error.code;
			}
			
			
			robotNew.retention_amout = reminMoney;
			robotNew.amount =  Double.parseDouble(remandAmount);
...
//			robotNew.status = Constants.IPS_ENABLE ? false : true;
			...
			robot.retention_amout = reminMoney;
			robot.amount =  Double.parseDouble(remandAmount);
...
//			robot.status = Constants.IPS_ENABLE ? false : true;
	 ...
	 * @return
	 */
	public static List<Invest> queryRegularInvestUser(long bidId) {
		List<Map<Long, Object>> tamounts = null;
		List<Invest> amounts = new ArrayList<Invest>();

		String hql = "select new Map(i.id as id,i.user_id as userId, i.amount as amount, i.mer_bill_no as mer_bill_no, i.ips_bill_no as ips_bill_no, i.fee as fee) from t_invests i where i.bid_id=? and loan_bid_status=? order by time";

		try {
			tamounts = t_invests.find(hql, bidId,0).fetch();
		} catch (Exception e) {
	 ...
	 * @return
	 */
	public static List<Invest> queryAllRegularInvestUser(long bidId) {
		List<Map<Long, Object>> tamounts = null;
		List<Invest> amounts = new ArrayList<Invest>();

		String hql = "select i.id as id from t_invests i where i.bid_id=?  order by time";
	...
	public static void updateReadCount(long bidId,ErrorInfo error){
		error.clear();
		int row =0;
	    String loan_sql ="update t_bids set read_count = read_count + 1 where id = ?";

		Query query = JPA.em().createQuery(loan_sql);
		query.setParameter(1, bidId);//状态
		try {
			row = query.executeUpdate();
		} catch (Exception e) {
			e.printStackTrace();
			Logger.error("标的详情->更新浏览次数:" + e.getMessage());
			error.msg = "标的详情->更新浏览次数 修改状态失败!";
			JPA.setRollbackOnly();
...
//            page.totalCount = QueryUtil.getQueryCountByCondition(em, sql.toString(), params);
	 ...
	 * @return
	 */
	public static List<v_receiving_invest_bids> queryUserReceivingInvestBids(long userId,ErrorInfo error){
		error.clear();
		...
		EntityManager em = JPA.em();
		List<v_receiving_invest_bids> vbid =null;
		try {
			sql.append(" order by id desc");
            Query query = em.createNativeQuery(sql.toString(),v_receiving_invest_bids.class);
	 ...
	 * @return
	 */
	public static void redeemSuccess(Invest invest,boolean flag,long auditId,ErrorInfo error){
		error.clear();
		//赎回操作
		t_bill_invests bi=t_bill_invests.find(" invest_id=?",invest.id).first();
		if(bi!=null){
			
			Bid bid=new Bid();
			bid.id=bi.bid_id;
//			//算利息的天数
//			int days = DateUtil.daysBetween(bid.realInvestExpireTime,new Date());
//			//天利息  deadline 借款天标的天数  borrowSum  借款的总额 monthRate  月利率  
//			double monPayInterest = Arith.mul(Arith.mul(Arith.div(bid.apr/100, 365,6), bi.receive_corpus), days);//申请赎的回标的总利息
//			//本次赎回 还款金额  
//			double  money = invest.amount + monPayInterest;
            //增加投资者资金 若投资者提前赎回资金则只返还本金，正常还款才有利息
			int rows = DealDetail.addUserFund(invest.userId, invest.amount);
		
			if (rows < 1) {
				error.msg = "定期标赎回增加投资者资金失败!";
				JPA.setRollbackOnly();
			...
			//及时查询用户最新状态
			detail = DealDetail.queryUserFund(invest.userId, error);
			
			DealDetail dealDetail = new DealDetail(invest.userId, DealType.ADD_NOMAL_REGULAR,
					invest.amount, invest.bidId, detail.get("user_amount"), detail.get("freeze"),
					0.00, "获得定期标投资赎回金额");

			dealDetail.addDealDetail(error);
			
			//用户事件添加
			DealDetail.userEvent(invest.userId, UserEvent.ADD_SHUHUI, "投资金额赎回成功", error);
			
			/* 添加平台交易记录 */
			dealDetail.addPlatformDetail(DealType.PLATFORM_TO_USER, invest.userId, -1, invest.userId,  
					DealType.ACCOUNT, invest.amount, 2,
					"支出用户赎回金额", error);
			
			if (error.code < 0) {
				JPA.setRollbackOnly();
				error.msg = "支出用户赎回金额失败";

				return;
			}
			//修改理财账单状态
			Bill.updateBillStatus(bi.id,error);
			
			//修改投资表 赎回审核状态
			updateInvestRedeemStatus(bi.invest_id, Constants.TWO , auditId,error);
			...
			email.setId(Templets.E_REGULAR_REDEEM_SUCCESS);
			station.setId(Templets.M_REGULAR_REDEEM_SUCCESS);
		
			
			if(station.status){
				String	scontent = station.content;
				scontent = scontent.replace("userName", invest.user.name);
				scontent = scontent.replace("date", DateUtil.dateToString((new Date())));
				scontent = scontent.replace("title", invest.bid.title);
				scontent = scontent.replace("money", invest.amount+"");

				StationLetter letter = new StationLetter();
				letter.senderSupervisorId = 1;
				letter.receiverUserId = invest.userId;
			...
			if(email.status){
				...
				econtent = econtent.replace("userName", invest.user.name);
				econtent = econtent.replace("date", DateUtil.dateToString((new Date())));
				econtent = econtent.replace("title", invest.bid.title);
				econtent = econtent.replace("money", invest.amount+"");
				
				email.addEmailTask(invest.user.email, email.title, econtent);
			}
			if(flag){
				
				TemplateSms sms = new TemplateSms();
				sms.setId(Templets.S_REGULAR_REDEEM_SUCCESS);
				
				if(sms.status && StringUtils.isNotBlank(invest.user.mobile)){
					String 	smscontent = sms.content;
					smscontent = smscontent.replace("userName", invest.user.name);
					smscontent =smscontent.replace("date", DateUtil.dateToString((new Date())));
					smscontent = smscontent.replace("title", invest.bid.title);
					smscontent = smscontent.replace("money", invest.amount+"");
					sms.addSmsTask(invest.user.mobile, smscontent);
				}
			}
		}
	 ...
	 * @return
	 */
	public static void redeemFail(Invest invest , long auditId, ErrorInfo error){
		error.clear();
		//赎回不通过操作
		//修改投资表 赎回审核状态
		updateInvestRedeemStatus(invest.id, Constants._ONE ,auditId , error);
	 ...
	 * @return
	 */
	public static void updateInvestRedeemStatus(long id,int statu , long auditId,ErrorInfo error){
		error.clear();
		int row =0;
	    String loan_sql ="update t_invests set redeem_status =? , redeem_audit_id = ? where id=?  ";

		Query query = JPA.em().createQuery(loan_sql);
		query.setParameter(1, statu);//赎回状态
		query.setParameter(2, auditId);//审核人id 
		query.setParameter(3, id);// 投资记录
		try {
			row = query.executeUpdate();
		} catch (Exception e) {
			e.printStackTrace();
	...
	public static void investBbin(long userId, long bidId, int investTotal, String dealpwdStr, boolean isAuto, int client, ErrorInfo error) {
		error.clear();	
		User user = new User();
	    	user.id = userId;
	    	//查询体验金账户信息
	    	t_bbin tbbin = Bbin.queryBbinforInvest(userId, error);
			if (investTotal <= 0) {
			...
			if( time> time2){
				...
				error.code = -2;
				JPA.setRollbackOnly();		Logger.error("-----------111Incest1767bidId:%s,time:%s ,invest_expire_time.getTime:%s-------------",bidId,time,time2);		
				return;
			}
			if (userId == bidUserId) {
			...
			if (is_deal_password == true && !Constants.IPS_ENABLE  && !isAuto) {
			...
			if (averageInvestAmount == 0) {
				...
				if (tbbin.account < investTotal ) {
					error.msg = "对不起！您可用体验金余额不足！根据您的余额您最多只能投" + tbbin.account + "元";
					error.code = Constants.BBIN_NOT_ENOUGH;
					return;
				}
				if (investTotal > (amount - hasInvestedAmount)) {
					double money = amount - hasInvestedAmount;
			...
			if (minInvestAmount == 0) {
				...
				if (tbbin.account < investTotal) {
					error.msg = "对不起！您余额不足！您最多只能购买"
							+ (int) (tbbin.account/ averageInvestAmount) + "份！";
					error.code = Constants.BBIN_NOT_ENOUGH;
					return;
				}
			}
			...
			doInvestBbin(user1, bidId, investTotal, pMerBillNo, pFee, client, award, bid_fee, error);
		}
	 ...
	 * @param error
	 */
	public static void invest(long userId, long bidId, int investTotal, String dealpwdStr, boolean isAuto, int client, ErrorInfo error) {
		error.clear();	
		User user = new User();
    	user.id = userId;
		...
		double award = map.get("award");
		double bid_fee = map.get("bid_fee") ;
		doInvest(user1, bidId, investTotal, pMerBillNo, pFee, client, award, bid_fee, error);
	}
	public static void doInvestBbin(t_users user1, long bidId, double investTotal, String pMerBillNo, double pFee, int client, double award, double bid_fee, ErrorInfo error) {
		error.clear();
		...
		/* 体验金额减少,冻结资金增加 */
		DealDetail.freezeFundBbin(userId, investTotal, error);
		if (error.code < 0) {
			return;
		}
		// 更新会员性质
		...
		double user_amount = tbbin.account;
		double freeze = tbbin.invested_account;
		double receive_amount = tbbin.invested_account;	
		// 添加交易记录
		DealDetail dealDetail = new DealDetail(userId, DealType.FREEZE_INVEST, investTotal, bidId, user_amount, 
				freeze, receive_amount, "冻结投标金额");
		dealDetail.addDealDetail(error);
		if (error.code < 0) {
			JPA.setRollbackOnly();
		...
		if(error.code < 0) {
		 ...
		 if(sms.status && StringUtils.isNotBlank(user1.mobile)){
			 ...
			 smscontent = smscontent.replace("description", "按天反息,到期收本!");
...
//						PushMessage.pushUnicastMessage(bill.device_user_id, bill.channel_id, bill.device_type, device);
	 ...
	 * @param error
	 */
	public static void doInvest(t_users user1, long bidId, double investTotal, String pMerBillNo, double pFee, int client, double award, double bid_fee, ErrorInfo error) {
		error.clear();
...
//						PushMessage.pushUnicastMessage(bill.device_user_id, bill.channel_id, bill.device_type, device);
	...
	public static Map<String,Object> queryBiderByParam(t_user_automatic_invest_options autoOptions,int unit,long bidId){
		...
		if(unit == -1){//单位为年
			min_period = autoOptions.min_period*12*30;
			max_period = autoOptions.max_period*12*30;
		}
		
		if(unit == 0 ){
			min_period = autoOptions.min_period*30;
			max_period = autoOptions.max_period*30;
		}
		if(unit == 1 ){
		...
		condition.append("select new Map(id as id) from v_confirm_autoinvest_bids where apr >= "
				+ autoOptions.min_interest_rate + "  and apr <= " + autoOptions.max_interest_rate);
...
//			}
	...
	public static void automaticInvest() {
		...
		for(Map<String, Object> map : biderList) {
			...
			for(Long userId2 : userIds) {
				...
				unit = Integer.parseInt(map.get("period_unit").toString());//0
				...
				double balance = Invest.queryAutoUserBalance(userId2, userParam.retention_amout);//减去用户设置的保留余额后的 用户可投资金额
				double setAmount = Double.parseDouble(map.get("min_invest_amount").toString());//用户设置的投标金额(标的最低投标金额)
				...
				try {
					invest(userId2, bidId, bidAmount, "", true, Constants.CLIENT_PC, error);
				} catch (Exception e) {
	 ...
	 * @return
	 */
	public static List<v_front_all_bids> queryAgencyBbids(){
	 ...
	 * @return
	 */
	public static List<v_front_all_bids> queryRegularBids(){
		
		List<v_front_all_bids> regularBids = null;
		StringBuffer sql = new StringBuffer("");
		sql.append(SQLTempletes.SELECT);
		sql.append(SQLTempletes.V_FRONT_ALL_BIDS);
		sql.append(" and t_bids.is_regular = 1");
		sql.append(" order by t_bids.time desc ");
		List<Object> params = new ArrayList<Object>();
		...
		try {
            ...
            regularBids = query.getResultList();
	...
	public static void creatSingleInvestPact(long investId, ErrorInfo error){
		...
		content = content.replace(Templets.AMOUNT,bidbusiness.amount + "")
	 ...
	 * @return
	 */
	public static PageBean<v_front_all_bids> queryAllBidsNotRepay(int showType, int currPage,int pageSize,String _apr,String _amount,String _loanSchedule,String _startDate,String _endDate,String _loanType,String minLevelStr,String maxLevelStr,String _orderType,String _keywords,ErrorInfo error){
...
//				params.add("%"+_keywords+"%");
	 ...
	 * @return
	 */
	public static Date querryDeadLineTime(long investId,long bidId,ErrorInfo error){
		String investsSql="SELECT deadline_time FROM t_bid_loantime_record where invest_id="+investId+" and bid_id="+bidId+"";
		EntityManager em = JPA.em();
		Object record = null;
		...
		try {
			...
			Logger.error("查询按天返息顺延期限时：" + e.getMessage());
		}
		Date  sumInvests = null;
		if (null != record) {
			sumInvests = DateUtil.strToDate(record.toString());
		}
	 ...
	 * @return
	 */
	public static List<t_bid_imagefile_name> querryBidPhotos(long bidId,ErrorInfo error){
		String sql = "select * FROM t_bid_imagefile_name where bid_id = "+bidId+" order by upload_time desc";
		EntityManager em = JPA.em();
		List<t_bid_imagefile_name> imageList = null;
//		t_bid_imagefile_name image = null;
		Query query  = em.createNativeQuery(sql,t_bid_imagefile_name.class);
		
		try {
			imageList = query.getResultList();
		} catch (Exception e) {
			Logger.error("查询标的图片时：" + e.getMessage());
		}
